...
struct node *list_create_node(void *elt) {
    ...
    if (r == NULL) error(2, 0, "malloc failed in create_node\n");
    r->elt = elt;
    return r;
}
...
struct list *list_create(void) {
    ...
    if (r == NULL) error(2, 0, "malloc failed in list_create\n");
    r->length = 0;
    r->first = NULL;
    r->last = NULL;
    return r;
...
int list_is_empty(const struct list *l) { return l->length == 0; }

size_t list_length(const struct list *l) { return l->length; }
...
struct list *list_add(struct list *l, void *elt) {
    if (list_is_empty(l)) return list_add_in_empty(l, elt);
    struct node *n = list_create_node(elt);
    ++(l->length);
    l->last->next = n;
    n->prev = l->last;
...
struct list *list_add_index(struct list *l, void *elt, size_t i) {
    if (i > l->length)
        error(1, 0, "list_add_index: index %zu out of bound %zu\n", i,
              l->length);
    if (list_is_empty(l)) return list_add_in_empty(l, elt);
    if (i == list_length(l)) return list_add(l, elt);
    if (i == 0) return list_add_first(l, elt);
...
void *list_remove(struct list *l) { return list_remove_node(l, l->last); }
